...
function initFilePanel() {
  ...
  setTooltip(diffToggleBtn, diffMode === 'inline' ? 'Switch to side-by-side diff' : 'Switch to inline diff');
  ...
  setTooltip(diffSaveBtn, 'Save changes');
  ...
  setTooltip(diffCloseBtn, 'Close panel');
  ...
  // This toolbar is built by hand rather than through createViewerToolbar, so it
  // needs wiring up to the shared tooltip layer explicitly.
  attachTooltips(diffToolbarEl);
